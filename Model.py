...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False):
        ...
        assert n_state % config.n_heads == 0
...
class FeedForward(nn.Module):
    def __init__(self, n_state, config):  # in FeedForward: n_state=3072 (4 * n_embed)
...
class Block(nn.Module):
    def __init__(self, n_ctx, config, scale=False):
        ...
        self.mlp = FeedForward(4 * nx, config)
...
class GPT2S(nn.Module):
    def __init__(self, config):
        super(GPT2S, self).__init__()
        self.transformer = GPT2Model(config)
        self.lm_head = LMHead(self.transformer.wte.weight, config)
